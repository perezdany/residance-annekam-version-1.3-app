<?php
   ...
   // require '../phps/confige.php';
	...
	if(isset($_SESSION['appart']) AND isset( $_SESSION['nom']) AND isset($_SESSION['pnom']) AND isset($_SESSION['date']) AND isset($_SESSION['newdate1']) AND isset($_SESSION['montant']) AND isset($_SESSION['desig'])  AND isset( $_SESSION['j']) AND isset($_SESSION['reserv']))
	 {
	 	...
	 	//creation du document on ouvre une session ou une instance de fpdf
	...
	if($_SESSION['s']==0)//c'est une facture proformat
		{
			...
			$pdf->Cell(20, 10,'','B', 0);
			$pdf->Cell(20, 10,''.number_format($_SESSION['montant'], 0, ',', ' ').'','B', 1);
		...
		$pdf->Cell(15,10,'','', 0);
		$pdf->SetFont('courier','B',12);
		$pdf->Cell(168, 10,utf8_decode('Nos références'),'TB', 1, 'C');
		$pdf->Cell(0, 10,'','', 1);
	...
	{
		echo'Oups une erreur est survenue<a href="../register.php">retour</a>';
	}
