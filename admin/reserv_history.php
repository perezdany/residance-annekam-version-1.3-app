...
<head>
  ...
  <title> Admin | Historique des réservations</title>
        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Historiques des réservations</h6>
            </div>
            <div class="card-body">
              <div class="table-responsive" id="section_to_print">
                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0" >
	                <thead>
	                    <tr>
	                      <th>cliquer</th><th>Appartement</th><th><span>Client</span></th><th><span>Montant</span><th><span>Date de la réservation</span><th><span>Statut</span></th><th><span>Date de départ</span></th><th><span>ACTION</span></th>
	                    </tr>
	                </thead>
	                <tbody>
	                  <?php
	                  	//affichage des apparts
	                  	$see = (new reservation())->reserv_history();

                      while($v = $see->fetch())
                      {
                        $timestampd = strtotime($v[1]);//recuperation du timestanp de la date donnée
                        $new_format = date("d/m/Y", $timestampd);//changement du format
                        $timestampd1 = strtotime($v[10]);//recuperation du timestanp de la date donnée
                        $new_format1 = date("d/m/Y", $timestampd1);//changement du format
                        $timestampd2 = strtotime($v[16]);//recuperation du timestanp de la date donnée
                        $new_format2 = date("d/m/Y", $timestampd2);//changement du format
                            echo'<tr><td align="center" style="width:220px">
                                ...
                                <div class="content" style="display:none;">
                                  Tarifs mensuel de l\'appartement: '.$v[15].'<br>
                                  Nombre de jour: '.$v[3].'<br>
                                  Heure de réservation: '.$v[2].'<br>
                                  Date d\'arrivée: '.$new_format2.'<br>
                                  Objet de la réservation: '.$v[11].'
                                  Remise: '.$v[17].'
                                </div>
                            </td><td>'.$v[7].'</td><td>'.$v[13].' '.$v[4].' '.$v[5].'</td><td>'.number_format($v[8], 0, ',', ' ').'</td>';
                          ...
                          echo'<td>'.$new_format1.'</td>';
                            echo'
                              <td align="center">
                                <button type="button" class="collapsible btn btn-circle bg-gradient-success"><font color="white"><i class="fa fa-angle-down"></i></font></button>
                                <div class="content" style="display:none;">
                                  <form method="post" action="../Model/delete.php"><input type="text" value="'.$v[0].'" style="display: none;" name="id"><button class="btn btn-circle bg-gradient-danger" type="submit" name="delresh"><font color="white"><i class="fa fa-trash"></i></font></button>
                                  </form>
                                  <button class="btn btn-circle bg-gradient-info" type="submit"><a href="../facture/impreservation.php?id='.$v[0].'&amp;datres='.$v[1].'&amp;hres='.$v[2].'&amp;j='.$v[3].'&amp;nc='.$v[4].'&amp;pnc='.$v[5].'&amp;s='.$v[6].'&amp;a='.$v[7].'&amp;m='.$v[8].'&amp;datar='.$v[9].'&amp;datdep='.$v[10].'&amp;ob='.$v[11].'&amp;ob='.$v[11].'&amp;rem='.$v[14].'"><font color="white"><i class="fa fa-print"></i></font></a>
                                  </button>';
                                      
                                  echo'<button class="btn btn-circle bg-gradient-info">
                                        <a href="edit_reservation.php?id='.$v[0].'&amp;clt='.$v[12].'&amp;r='.$v[11].'&amp;statut='.$v[6].'"><font color="white"><i class="fa fa-edit"></i></font>
                                        </a>
                                      </button>
                                      <button class="btn btn-circle bg-gradient-primary">
                                        <a href="buy.php?id='.$v[0].'&amp;clt='.$v[12].'&amp;r='.$v[11].'"><font color="white"><i class="fas fa-dollar-sign"></i></font>
                                        </a>
                                      </button>';
