            ...
            <div class="card-body">
              <div class="table-responsive" id="section_to_print">
                <table class="table table-bordered" id="" width="100%" cellspacing="0">
                  <thead>
                    <tr><th>Réservation du:</th><th>Date d'emission</th><th>Montant(FCFA)</th><th>Reste à payer (FCFA)</th><th>Heure d'émission</th><th>Action</th></tr>
                  </thead>
                  <tbody>
                    <?php
                      $voir = (new facture())->all_fact();

                      while($v = $voir->fetch())
                      {
                        $timestampd1 = strtotime($v[14]);//recuperation du timestanp de la date donnée
                        $new_format1 = date("d/m/Y", $timestampd1);//changement du format

                        $timestampd2 = strtotime($v[3]);//recuperation du timestanp de la date donnée
                        $new_format2 = date("d/m/Y", $timestampd2);//changement du format
                          echo'<tr><td>'.$new_format1.'</td><td>'.$new_format2.'</td><td>'.number_format($v[9], 0, ',', ' ').'</td><td>'.number_format($v[4], 0, ',', ' ').'</td><td>'.$v[5].'</td>';
                          ...
                          echo'<td>
                              ...
                              <div class="content" style="display:none;">
                                <button class="btn btn-circle bg-gradient-success" type="submit" name="del"><a href="../facture/facture.php?d='.$v[3].'&amp;i='.$v[0].'&amp;n='. $v[12].'&amp;p='. $v[13] .'&amp;a='.$v[8].'&amp;j='.$v[10].'&amp;tj='.$v[7].'&amp;pay='.$v[1].'&amp;rest='.$v[4].'&amp;mr='.$v[9].'&amp;tm='.$v[11].'&amp;s='.$v[6].'&amp;rs='.$v[15].'"><font color="white"><i class="fa fa-print"></i></font></a>
                                </button>
                                <form method="post" action="../Model/delete.php"><input type="text" value="'.$v[0].'" style="display: none;" name="id"><button class="btn btn-circle bg-gradient-danger" type="submit" name="delf"><font color="white"><i class="fa fa-trash"></i></font></button>
                                </form>
                              </div>
                            </td>
                          </tr>';
